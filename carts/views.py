...
def cart(request, total=0, quantity=0, cart_items=None):
    try:
        ...
        for cart_item in cart_items:
            category_name = cart_item.product.category.category_name.replace("-", "").replace(" ", "").lower()
            if "tshirt" in category_name or "kurti" in category_name:
                discounted_price = int(cart_item.product.price * 0.5)
            else:
